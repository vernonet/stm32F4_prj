...
extern volatile int			bytes_left, bytes_left_old;
extern char					*read_ptr, *read_ptr_old;
...
void PlayAudioWithCallback(AudioCallbackFunction *callback, void *context) {
	...
	if (CallbackFunction) {
		err = CallbackFunction(CallbackContext, BufferNumber);
		if (err==-2 || err==-9) {
			read_ptr+=0x1a1;
			bytes_left-=0x1a1;
			err = CallbackFunction(CallbackContext, BufferNumber);
...
static void StartAudioDMAAndRequestBuffers() {
	...
	DMA1_Stream7 ->CR = (0 * DMA_SxCR_CHSEL_0 ) | // Channel 0
			(1 * DMA_SxCR_PL ) | // Priority 1      //DMA_SxCR_PL_0   //PRIORITY_VERYHIGH   DMA_SxCR_PL
